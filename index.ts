  ...
  ws.on("message", async function (message: string) {
    if (message != "undefined") {
      ...
      if (params.connect == true) {
        ...
        console.log(params.order, 888)
        if (params.order == "mopai") { // 摸牌指令
          const res = await axiod.get(
            "http://127.0.0.1:7145/qingshu/mopai?id=" + params.id,
          );
          if (res.data.code == 200) {
            const res2 = await axiod.get("http://127.0.0.1:7145/qingshu/getGameData");
            for (let i = 0; i < idList.length; i++) {
              objList[i].send(JSON.stringify(res2.data));
            }
          }
        } else if(params.order == "reset") { // 重置指令
          const res = await axiod.get("http://127.0.0.1:7145/qingshu/getGameData");
          for (let i = 0; i < idList.length; i++) {
            objList[i].send(JSON.stringify(res.data));
          }
        } else if(params.order == "chupai") { // 出牌指令
          const res = await axiod.get(
            "http://127.0.0.1:7145/qingshu/chupai?id=" + params.id + "&wz=" + params.wz,
          );
          if (res.data.code == 200) {
            const res2 = await axiod.get("http://127.0.0.1:7145/qingshu/getGameData");
            for (let i = 0; i < idList.length; i++) {
